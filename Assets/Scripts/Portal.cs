...
public class Portal : MonoBehaviour
{
    ...
    public bool PlacePortal(Collider wallCollider, Vector3 pos, Quaternion rot)
    {
        ...
        Debug.Log($"Placing portal at {pos}, Rotation: {rot.eulerAngles}");
        Debug.Log($"Adjusted position after forward offset: {testTransform.position}");

        FixOverhangs();
        Debug.Log($"After FixOverhangs: Position: {testTransform.position}, Rotation: {testTransform.rotation.eulerAngles}");

        FixIntersects();
        Debug.Log($"After FixIntersects: Position: {testTransform.position}, Rotation: {testTransform.rotation.eulerAngles}");
        ...
        if (CheckOverlap())
        {
            ...
            IsPlaced = true;
            Debug.Log($"Portal successfully placed at {testTransform.position} with Rotation: {testTransform.rotation.eulerAngles}");
            return true;
        }
        else
        {
            Debug.LogError($"Portal placement failed due to overlap at Position: {testTransform.position}, Rotation: {testTransform.rotation.eulerAngles}");
            return false;
        }
    ...
    private bool CheckOverlap()
    {
        ...
        Vector3 forwardOffset = testTransform.TransformVector(new Vector3(0.0f, 0.0f, 0.2f));
        ...
        if (intersections.Length > 1 || (intersections.Length == 1 && intersections[0] != collider))
        {
            return false;
        }

        // Check corners overlap
        for (int i = 1; i < checkPositions.Length; ++i)
        {
            if (!Physics.Linecast(checkPositions[i], checkPositions[i] + forwardOffset, placementMask))
            {
                return false; // At least one corner doesn't overlap
            }
        }
